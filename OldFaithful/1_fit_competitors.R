...
# args <-  c(2, 100, 220816)
p_use <- as.numeric(args[1]) # covariates: 1 => (w1, w2), 3 => (w1, d1), 4 => (w1, w2, d1)
ii <- as.numeric(args[2]) # train/test set replicate id
datenow <- as.numeric(args[3]) # random seed based on date integer
...
mse <- numeric(n_compet) # in-sample mean squared error
mspe <- numeric(n_compet) # out-of-sample mean squared error

mae <- numeric(n_compet) # in-sample mean absolute error
mape <- numeric(n_compet) # out-of-sample mean absolute error

mld <- numeric(n_compet) # in-sample mean log-density evaluations (-2 times this is deviance)
mpld <- numeric(n_compet) # out-of-sample mean log-density evaluations (-2 times this is deviance)

mdld <- numeric(n_compet) # in-sample median log-density evaluations (-2 times this is deviance)
mdpld <- numeric(n_compet) # out-of-sample median log-density evaluations (-2 times this is deviance)

ksd <- numeric(n_compet) # in-sample Kolmogorov-Smirnov distance of empirical CDF of quantile residuals from uniform distribution
kspd <- numeric(n_compet) # out-of-sample Kolmogorov-Smirnov distance of empirical CDF of quantile residuals from uniform distribution

names(mse) <- names(mspe) <- competitors
names(mae) <- names(mape) <- competitors
names(mld) <- names(mpld) <- competitors
names(mdld) <- names(mdpld) <- competitors
...
## centered/scaled response and covariates
ysc_tn <- ysc[indx_tn]
ysc_tt <- ysc[indx_tt]
...
if (p_use < 3) {
  Xsc_imp_rf <- missForest( cbind(Xsc_imp0, trash=rnorm(nrow(Xsc_imp0))) )$ximp[,1:ncov]  
} else {
...
mdld["BART"] <- apply(sapply(1:bart_nsim, function(i) dnorm(ysc_tn, bart_pred_insamp$y_hat_posterior_samples[,i], sqrt(bart_sig2samp[i]), log=TRUE)), 1, median) |> median()
mdpld["BART"] <- apply(sapply(1:bart_nsim, function(i) dnorm(ysc_tt, bart_pred$y_hat_posterior_samples[,i], sqrt(bart_sig2samp[i]), log=TRUE)), 1, median) |> median()
...
if ( any(is.na(rbind(Xsc_tn, Xsc_tt))) ) { 
...
mdld["MI"] <- lapply(mi.fits, function(x) {
  (sapply(1:x$nsim, function(i) dnorm(ysc_tn, x$fitted_sim[i,], x$sigma_sim[i], log=TRUE)))
} ) %>% do.call(cbind, .) %>% apply(., 1, median) %>% median()

mdpld["MI"] <- lapply(mi.fits, function(x) {
  (sapply(1:x$nsim, function(i) dnorm(ysc_tt, x$preds_sim[i,], x$sigma_sim[i], log=TRUE)))
} ) %>% do.call(cbind, .) %>% apply(., 1, median) %>% median()
...
save(mse, mspe, mae, mape, mld, mpld, mdld, mdpld, ksd, kspd, p_use, ii, datenow, datname,
     file=paste0("postsim/summary_", datname, "_competitors_p", p_use, "_ii", ii, "_date", datenow, ".rda"))
